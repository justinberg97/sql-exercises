...
FROM restaurants
LEFT JOIN menus ON menus.restaurant_id = restaurants.id
LEFT JOIN menuItems ON menuItems.menu_id = menus.id;
-- Construct the following query:
-- All restaurants
-- Include their menus
-- Include the menu items
-- We want ALL results:
-- even if a restaurant has no menus
-- even if a menu has no menu items
